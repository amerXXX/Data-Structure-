...
public class BST<T extends Comparable<T>> extends BinaryTree<T> {
    protected dictionary.BTNode<T> root;
    public BST() {
    }

    public BST(dictionary.BTNode root) {
         this.root = root;
    }
    ...
    public void insert(T el) {
        dictionary.BTNode<T> p = root, prev = null;
        ...
        if (root == null)    // tree is empty;
             root = new dictionary.BTNode<T>(el);
        else if (el.compareTo(prev.data) < 0)
             prev.left  = new dictionary.BTNode<T>(el);
        else 
             prev.right = new dictionary.BTNode<T>(el);
    }
   
    public boolean search(T el) {               
        dictionary.BTNode<T> p = root;
        while (p != null)
            if (el.equals(p.data))                
                 return  true;                     
    ...
    public void deleteByCopying(T el) {
        dictionary.BTNode<T> node, p = root, prev = null;
        ...
        if (p != null && p.data.equals(el)) {
             if (node.right == null)             // node has no right child;
                  ...
                  dictionary.BTNode<T> tmp = node.left;    // node has both children;
                  dictionary.BTNode<T> previous = node;    // 1.
             ...
             throw new NoSuchElementException("el " + el + " is not in the tree");
        else 
            throw new UnsupportedOperationException("the tree is empty");
    }
    public void deleteByMerging(T el) {
        dictionary.BTNode<T> tmp, node, p = root, prev = null;
        while (p != null && !p.data.equals(el)) {  // find the node p
             prev = p;                           // with element el;
             if (el.compareTo(p.data) < 0)
                  p = p.right;
    ...
    public void iterativePreorder() {
        dictionary.BTNode<T> p = root;
        Stack<dictionary.BTNode<T>> travStack = new Stack<dictionary.BTNode<T>>();
        if (p != null) {
             travStack.push(p);
    ...
    public void iterativePostorder2() {
        dictionary.BTNode<T> p = root;
        Stack<dictionary.BTNode<T>> travStack = new Stack<dictionary.BTNode<T>>(),
                          output = new Stack<dictionary.BTNode<T>>();
    ...
    public void iterativePostorder() {
        dictionary.BTNode<T> p = root, q = root;
        Stack<dictionary.BTNode<T>> travStack = new Stack<BTNode<T>>();
        while (p != null) {
